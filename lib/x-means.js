export default class XMeans {
    ...
    improveParameters(centers, availableIndexes = null) {
        ...
        while (changes > stopCondition) {
            ...
            console.log('before update centers: ', clusters);
    ...
    updateClusters(centers, availableIndexes = null) {
        ...
        for (let indexPoint = 0; indexPoint < bypass.length; indexPoint++) {
            ...
            for (let index = 0; index < centers.length; index++) {
                console.log('distance of -- ', indexPoint, index, this.pointerData[indexPoint], centers[index]);
            ...
            console.log(`${indexOptim} , ${clusters[indexOptim]}`, clusters);
        }
    ...
    updateCenters(clusters) {
        ...
        for (let index = 0; index < clusters.length; index++) {
            ...
            for (let i = 0; i < clusters[index].length; i++) {
                ...
                console.log('++++++++++', indexPoint, pointSum, this.pointerData[indexPoint]);
    ...
    euclideanDistanceSquared(a, b) {
        ...
        let res = a.map((_, i) => Math.pow(a[i] - b[i], 2)).reduce((prev, next) => prev + next, 0);
    ...
    getClusterData() {
        return this.clusters.map(index => this.pointerData[index]);
    }
}